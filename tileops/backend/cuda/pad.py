"""CUDA / HIP constant-value padding kernels.

Two kernels are provided:

* ``pad1d`` — pad the last dimension only (e.g. sequence-length padding).
* ``pad2d`` — pad the last two dimensions (e.g. conv2d H/W padding).
...
The facade :mod:`tileops.pad` dispatches here for ``cuda`` / ``hip`` /
``cutedsl`` targets.
"""
...
def pad1d_kernel(
    ...
    """Pad the last dimension of a 2-D view ``(M, N)``.

    Output columns = *n* + *pad_left* + *pad_right*.  The host pre-fills the
    output with the constant value; this kernel copies input rows into the
    middle region of each output row.
    """
...
def pad2d_kernel(
    ...
    *,
    ...
    """Pad the last two dimensions of a 3-D view ``(M, H_in, W_in)``.

    Output shape is ``(M, H_out, W_out)`` with
    ``H_out = h_in + pad_top + pad_bottom``,
    ``W_out = w_in + pad_left + pad_right``.

    The host pre-fills the output with the constant value; this kernel copies
    input rows into the interior region of each 2-D slice.
    """
    h_out = h_in + pad_top + pad_bottom
    w_out = w_in + pad_left + pad_right
    n_out = h_out * w_out
...
_compile_cache = make_generic_cached_compiler(cache_prefix="cuda_pad_v1")
