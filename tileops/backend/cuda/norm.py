"""CUDA / HIP row-wise softmax, log-softmax, layer norm, RMS norm.
...
``M * N`` (and length *N* for per-channel weight / bias).

The facade :mod:`tileops.norm` dispatches here for ``cuda`` / ``hip`` /
``cutedsl`` targets.
"""
...
def softmax_kernel(m: int, n: int, _: int, dtype: Any, __eps: float) -> Any:
    # arg0_/arg1_/… keeps Metal buffer lexicographic order aligned with
    # host call order (matches cuda/metal norm modules).
    @T.prim_func
    def _kernel(
...
def layer_norm_kernel(m: int, n: int, _: int, dtype: Any, eps: float) -> Any:
    @T.prim_func
    def _kernel(
        arg0_X: T.Tensor((m * n,), dtype),
...
def rms_norm_kernel(m: int, n: int, _: int, dtype: Any, eps: float) -> Any:
    @T.prim_func
    def _kernel(
        arg0_X: T.Tensor((m * n,), dtype),
...
def skip_rms_norm_kernel(m: int, n: int, _: int, dtype: Any, eps: float) -> Any:
...
def skip_layer_norm_kernel(m: int, n: int, _: int, dtype: Any, eps: float) -> Any:
...
_NORM_PRIM: dict[str, Any] = {
    ...
    "skip_rms_norm": skip_rms_norm_kernel,
    "skip_layer_norm": skip_layer_norm_kernel,
}

_compile_cache = make_generic_cached_compiler(cache_prefix="cuda_norm_v3")
