"""CUDA reference 1-D and 2-D convolution (scalar accumulation loops).

``conv1d`` computes *C* = conv(*A*, *W*) with *A* ``(N, C_in, L)``, *W*
``(C_out, C_in // groups, kernel_size)``, output ``(N, C_out, L_out)``.
``conv2d`` computes the 2-D analogue.

Supports grouped convolution via the *groups* parameter (default 1).
Bias is required (pass zeros when there is no bias).
"""
...
_compile = make_generic_cached_compiler(cache_prefix="cuda_conv_v1")
...
def conv1d(
    ...
    *,
    ...
    return _compile(
        ("conv1d", N, C_out, C_in, L, kernel_size, stride, groups, dtype),
        conv1d_kernel,
        N, C_out, C_in, L, kernel_size, stride, groups, dtype,
        target=target,
...
def conv2d(
    ...
    *,
    ...
    return _compile(
        ("conv2d", N, C_out, C_in, H, W,
         kernel_h, kernel_w, stride_h, stride_w, groups, dtype),
        conv2d_kernel,
        N, C_out, C_in, H, W,
        kernel_h, kernel_w, stride_h, stride_w, groups, dtype,
        target=target,
