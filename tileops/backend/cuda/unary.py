"""CUDA / HIP mathematical unary kernels (TileLang ``@T.prim_func``).

These go beyond activations: :mod:`tileops.activation` covers the
activation-family unary ops; this module covers pure math operators:

  * **Exponential / logarithmic** — exp, log, exp2, exp10, log2, log10, log1p
  * **Trigonometry** — sin, cos, tan
  * **Inverse trigonometry** — asin, acos, atan
  * **Hyperbolic** — sinh, cosh, tanh (tanh in activation)
...
tail guard.  The facade :mod:`tileops.unary` dispatches here for
...
# Clamp takes min/max as compile-time constants (baked into the kernel).
# We do not expose parameterised clamp through the pattern below; the face
# module will use a dedicated compile path.
...
_compile = make_cached_compiler(_UNARY_PRIM, cache_prefix="cuda_unary")

# -- exponential / logarithmic -------------------------------------------
exp = build_op("exp", compile_fn=_compile, doc="CUDA/HIP: ``exp(x)``.")
log = build_op("log", compile_fn=_compile, doc="CUDA/HIP: ``log(x)`` (natural log).")
exp2 = build_op("exp2", compile_fn=_compile, doc="CUDA/HIP: ``exp2(x)`` (2**x).")
exp10 = build_op("exp10", compile_fn=_compile, doc="CUDA/HIP: ``exp10(x)`` (10**x).")
log2 = build_op("log2", compile_fn=_compile, doc="CUDA/HIP: ``log2(x)``.")
log10 = build_op("log10", compile_fn=_compile, doc="CUDA/HIP: ``log10(x)``.")
log1p = build_op("log1p", compile_fn=_compile, doc="CUDA/HIP: ``log1p(x)`` (log(1+x)).")

# -- trigonometry ---------------------------------------------------------
sin = build_op("sin", compile_fn=_compile, doc="CUDA/HIP: ``sin(x)``.")
cos = build_op("cos", compile_fn=_compile, doc="CUDA/HIP: ``cos(x)``.")
tan = build_op("tan", compile_fn=_compile, doc="CUDA/HIP: ``tan(x)``.")

# -- inverse trigonometry -------------------------------------------------
asin = build_op("asin", compile_fn=_compile, doc="CUDA/HIP: ``asin(x)``.")
acos = build_op("acos", compile_fn=_compile, doc="CUDA/HIP: ``acos(x)``.")
atan = build_op("atan", compile_fn=_compile, doc="CUDA/HIP: ``atan(x)``.")

# -- hyperbolic -----------------------------------------------------------
sinh = build_op("sinh", compile_fn=_compile, doc="CUDA/HIP: ``sinh(x)``.")
cosh = build_op("cosh", compile_fn=_compile, doc="CUDA/HIP: ``cosh(x)``.")

# -- inverse hyperbolic ---------------------------------------------------
asinh = build_op("asinh", compile_fn=_compile, doc="CUDA/HIP: ``asinh(x)``.")
acosh = build_op("acosh", compile_fn=_compile, doc="CUDA/HIP: ``acosh(x)``.")
atanh = build_op("atanh", compile_fn=_compile, doc="CUDA/HIP: ``atanh(x)``.")

# -- power / root --------------------------------------------------------
sqrt = build_op("sqrt", compile_fn=_compile, doc="CUDA/HIP: ``sqrt(x)``.")
rsqrt = build_op("rsqrt", compile_fn=_compile, doc="CUDA/HIP: ``rsqrt(x)`` (1/sqrt(x)).")
square = build_op("square", compile_fn=_compile, doc="CUDA/HIP: ``square(x)`` (x*x).")

# -- error function ------------------------------------------------------
erf = build_op("erf", compile_fn=_compile, doc="CUDA/HIP: ``erf(x)``.")

# -- sign / absolute -----------------------------------------------------
abs = build_op("abs", compile_fn=_compile, doc="CUDA/HIP: ``abs(x)``.")
sign = build_op("sign", compile_fn=_compile, doc="CUDA/HIP: ``sign(x)`` (-1, 0, or 1).")
neg = build_op("neg", compile_fn=_compile, doc="CUDA/HIP: ``neg(x)`` (-x).")

# -- special-value detection ----------------------------------------------
isnan = build_op("isnan", compile_fn=_compile, doc="CUDA/HIP: ``isnan(x)`` (1.0 if NaN).")
isinf = build_op("isinf", compile_fn=_compile, doc="CUDA/HIP: ``isinf(x)`` (1.0 if ±inf).")
isfinite = build_op("isfinite", compile_fn=_compile, doc="CUDA/HIP: ``isfinite(x)`` (1.0 if finite).")

# -- rounding ------------------------------------------------------------
round = build_op("round", compile_fn=_compile, doc="CUDA/HIP: ``round(x)`` (nearest integer).")
floor = build_op("floor", compile_fn=_compile, doc="CUDA/HIP: ``floor(x)``.")
ceil = build_op("ceil", compile_fn=_compile, doc="CUDA/HIP: ``ceil(x)``.")
trunc = build_op("trunc", compile_fn=_compile, doc="CUDA/HIP: ``trunc(x)`` (toward zero).")

# -- reciprocal ----------------------------------------------------------
reciprocal = build_op("reciprocal", compile_fn=_compile, doc="CUDA/HIP: ``reciprocal(x)`` (1/x).")

# -- bitwise --------------------------------------------------------------
bitwise_not = build_op("bitwise_not", compile_fn=_compile, doc="CUDA/HIP: ``bitwise_not(x)``.")

# -- clamp (special: compile-time min/max) -------------------------------

_clamp_compile = make_generic_cached_compiler(cache_prefix="cuda_unary_clamp")
