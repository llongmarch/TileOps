...
Mirrors :mod:`tileops.backend.cuda.norm` with a separate in-process cache prefix.
...
def online_softmax_kernel(m: int, n: int, _: int, dtype: Any, __eps: float) -> Any:
    """One-pass online softmax (same numerics as :func:`softmax_kernel`)."""
    @T.prim_func
    def _kernel(
        arg0_X: T.Tensor((m * n,), dtype),
        arg1_Y: T.Tensor((m * n,), dtype),
...
_compile_cache = make_generic_cached_compiler(cache_prefix="metal_norm_v3")
