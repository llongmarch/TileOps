"""Metal where and masked_fill — mirrors :mod:`tileops.backend.cuda.condition`."""
...
_cond_compile = make_cached_compiler(_COND_PRIM, cache_prefix="metal_condition")
where = build_op("where", compile_fn=_cond_compile)


# ── masked_fill (needs extra ``value`` compile-time parameter) ──────────

_mf_compile = make_generic_cached_compiler(cache_prefix="metal_condition_mf")
