...
def quantize_per_tensor_kernel(n: int, bs: int, t: int, dtype: Any) -> Any:
    @T.prim_func
    def _kernel(
...
def dequantize_per_tensor_kernel(n: int, bs: int, t: int, dtype: Any) -> Any:
    @T.prim_func
    def _kernel(
...
_POINTWISE_PRIM: dict[str, Any] = {
    "quantize_per_tensor": quantize_per_tensor_kernel,
    "dequantize_per_tensor": dequantize_per_tensor_kernel,
}
...
_CHANNEL_PRIM: dict[str, Any] = {
    "quantize_per_channel": quantize_per_channel_kernel,
    "dequantize_per_channel": dequantize_per_channel_kernel,
}

_ROW_PRIM: dict[str, Any] = {
    "per_token_quant_int8": per_token_quant_int8_kernel,
}

_compile = make_generic_cached_compiler(cache_prefix="metal_quant")
...
def _compile_pointwise(
    ...
    *,
    ...
    return _compile(
        ("v6", op_name, num_elements, block_size, threads, dtype),
        _POINTWISE_PRIM[op_name],
        num_elements, block_size, threads, dtype,
        target=target,
...
def _compile_row(
    ...
    *,
    ...
    return _compile(
        ("v7", op_name, rows, cols, block_size, threads, dtype, eps),
        _ROW_PRIM[op_name],
        rows, cols, block_size, threads, dtype, eps,
        target=target,
        execution_backend=execution_backend,
    )
...
def _compile_channel(
    ...
    *,
    ...
    return _compile(
        ("v6", op_name, rows, cols, block_size, threads, dtype),
        _CHANNEL_PRIM[op_name],
        rows, cols, block_size, threads, dtype,
        target=target,
        execution_backend=execution_backend,
    )
