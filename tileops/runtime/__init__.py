...
def validate_single_input(
    ...
    """Validate shape, dtype, and device for a single-input operator.

    Raises ``ValueError`` or ``TypeError`` with an *op*-tagged message
    when the tensors are incompatible.
    """
    if x.numel() == 0:
...
def _call_compiled_kernel(
    ...
    *inputs: torch.Tensor,
    ...
    """Call a compiled kernel with metal/torch vs non-metal dispatch.

    For ``metal`` + ``execution_backend="torch"`` the output tensor is passed
    as the last positional argument.  Other backends either return the output
    or accept it as the last positional argument.
    """
...
def _invoke_impl(
    ...
    *inputs: torch.Tensor,
    ...
    _call_compiled_kernel(kernel, *flat_inputs, out=out_flat,
                          kind=kind, execution_backend=execution_backend)
...
def invoke_quant_kernel(
    ...
    *x* and *out* share shape; *scale* is ``float32`` (length 1 or channel
    ...
    _call_compiled_kernel(kernel, xf, sf, out=of,
...
def invoke_gemm_kernel(
    ...
    *A* must be ``(M, K)``, *B* ``(K, N)``; output is ``(M, N)``.  Tensors must
    ...
    cc = out.contiguous()
    _call_compiled_kernel(kernel, aa, bb, out=cc,
                          kind=target_kind(tgt), execution_backend=eb)
...
def invoke_gemv_kernel(
    ...
    *A* is ``(M, K)``, *x* is ``(K,)``; output is ``(M,)``.
    ...
    yy = out.contiguous()
    _call_compiled_kernel(kernel, aa, xx, out=yy,
                          kind=target_kind(tgt), execution_backend=eb)
...
def invoke_row_reduce_kernel(
    ...
    *,
    ...
    _call_compiled_kernel(kernel, xf, out=of,
                          kind=target_kind(tgt), execution_backend=eb)
...
def invoke_row_index_reduce_kernel(
    ...
    *,
    ...
    _call_compiled_kernel(kernel, xf, out=oi,
                          kind=target_kind(tgt), execution_backend=eb)
...
def invoke_gather_kernel(
    ...
    *,
    ...
    """Run a compiled gather kernel: ``out[i] = data[index[i]]``.

    *data* is the source tensor, *index* is ``torch.int64``.
    Both keep their logical shapes (the facade permutes gather dim last).
    """
    if index.dtype != torch.int64:
        raise TypeError("invoke_gather_kernel: index must be torch.int64")
    if data.device != index.device:
        raise TypeError("invoke_gather_kernel: data and index must share device")

    df = data.contiguous().view(-1)
    indf = index.contiguous().view(-1)

    if out is None:
        out = torch.empty_like(index, dtype=data.dtype, device=data.device)
    else:
        if out.shape != index.shape:
            raise ValueError(
                f"invoke_gather_kernel: out.shape={tuple(out.shape)} != "
                f"index.shape={tuple(index.shape)}"
            )
    of = out.contiguous().view(-1)
    ...
    _call_compiled_kernel(kernel, df, indf, out=of,
                          kind=target_kind(tgt), execution_backend=eb)
    if of.data_ptr() != out.data_ptr():
        out.copy_(of)
    return out
...
def invoke_tril_triu_kernel(
    ...
    *,
    ...
    """Run a compiled ``tril`` / ``triu`` kernel (preserves 2-D shape)."""
    if x.ndim != 2:
        raise ValueError("invoke_tril_triu_kernel expects 2-D input")
    m, n = x.shape
    ...
    if out is None:
        ...
        if out.shape != (m, n):
            raise ValueError(
                f"invoke_tril_triu_kernel: out.shape={tuple(out.shape)} != ({m}, {n})"
            )
    ...
    _call_compiled_kernel(kernel, xc, out=oc,
                          kind=target_kind(tgt), execution_backend=eb)
...
def invoke_row_sort_kernel(
    ...
    *,
    ...
    """Run a compiled row-wise sort: flat *x* ``(M*N,)`` → flat *out* ``(M*N,)``."""
    if x_2d.ndim != 2:
        raise ValueError("invoke_row_sort_kernel expects 2-D input")
    m, n = x_2d.shape
    if out_1d.ndim != 1 or out_1d.numel() != m * n:
        raise ValueError(
            f"invoke_row_sort_kernel: out_1d must be 1-D of length {m * n}"
        )
    xf = x_2d.contiguous().reshape(-1)
    of = out_1d.contiguous().reshape(-1)
...
def invoke_conv_kernel(
    ...
    *input*, *weight* keep their logical shapes; *bias* is 1-D ``(C_out,)``.
    ...
    _call_compiled_kernel(kernel, inp_c, w_c, b_c, out=out_c,
                          kind=target_kind(tgt), execution_backend=eb)
...
def _make_runner_impl(
    ...
    *,
    ...
    def _run(*args: torch.Tensor) -> None:
        ...
        _call_compiled_kernel(kernel, *flat, out=out_flat,
                              kind=kind, execution_backend=execution_backend)
...
from tileops.runtime._infra import (  # noqa: E402
    ...
    dispatch_compile_pad,
    dispatch_compile_quant,
    dispatch_compile_reduce,
    dispatch_compile_softmax,
    build_op,
