...
exp = make_backend_op("exp", compile_fn=_compile, doc="CUDA/HIP: ``exp(x)``.")
log = make_backend_op("log", compile_fn=_compile, doc="CUDA/HIP: ``log(x)`` (natural log).")

# -- power / root --------------------------------------------------------
sqrt = make_backend_op("sqrt", compile_fn=_compile, doc="CUDA/HIP: ``sqrt(x)``.")
rsqrt = make_backend_op("rsqrt", compile_fn=_compile, doc="CUDA/HIP: ``rsqrt(x)`` (1/sqrt(x)).")
square = make_backend_op("square", compile_fn=_compile, doc="CUDA/HIP: ``square(x)`` (x*x).")

# -- sign / absolute -----------------------------------------------------
abs = make_backend_op("abs", compile_fn=_compile, doc="CUDA/HIP: ``abs(x)``.")
sign = make_backend_op("sign", compile_fn=_compile, doc="CUDA/HIP: ``sign(x)`` (-1, 0, or 1).")
neg = make_backend_op("neg", compile_fn=_compile, doc="CUDA/HIP: ``neg(x)`` (-x).")

# -- rounding ------------------------------------------------------------
round = make_backend_op("round", compile_fn=_compile, doc="CUDA/HIP: ``round(x)`` (nearest integer).")
floor = make_backend_op("floor", compile_fn=_compile, doc="CUDA/HIP: ``floor(x)``.")
ceil = make_backend_op("ceil", compile_fn=_compile, doc="CUDA/HIP: ``ceil(x)``.")

# -- reciprocal ----------------------------------------------------------
reciprocal = make_backend_op("reciprocal", compile_fn=_compile, doc="CUDA/HIP: ``reciprocal(x)`` (1/x).")
...
def clamp(
    ...
    *,
    ...
    key = ("cuda_clamp_v1", n, block_size, threads, dtype, tgt, execution_backend, lo, hi)
