...
blocks of *cols* elements) to one output value per row (length *rows*).
The facade merges ``dim`` to the last axis before calling here.
"""
...
def _compile(
    ...
    *,
    ...
    key = ("cuda_reduce_v1", op_name, rows, cols, threads, dtype, tgt,
           execution_backend)
    hit = _cache.get(key)
    if hit is not None:
        return hit
