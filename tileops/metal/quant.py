...
def _compile_pointwise(
    ...
    *,
    ...
    key = (
        "metal_quant_v6", op_name, num_elements, block_size, threads,
...
def _compile_row(
    ...
    *,
    ...
    tgt = target if target is not None else default_tilelang_target()
    key = (
        "metal_quant_v7", op_name, rows, cols, block_size, threads,
...
def _compile_channel(
    ...
    *,
    ...
    tgt = target if target is not None else default_tilelang_target()
    key = (
        "metal_quant_v6", op_name, rows, cols, block_size, threads,
        dtype, tgt, execution_backend,
    )
    hit = _cache.get(key)
    if hit is not None:
